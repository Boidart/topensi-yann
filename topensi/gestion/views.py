...
class IndexView(TemplateView):
    ...
    def get(self, request, **kwargs):
        ...
        return render(request, self.template_name,
                      {'info': info, 'client': client, 'partenaire': partenaire, 'type': type, 'etat': etat})
...
class AjouterInfoView(TemplateView):
    ...
    def post(self, request, **kwargs):
        ...
        try:
            if dateCloture == '':
                dateCloture = "1970-01"
            i = Info(cli=client_id, partenaire=partenaire_id, typ=type_id, etat=etat_id, marge=marge,
                     recurrent=recurrent, facture=facture, dateCloture=dateCloture, dateCreation=dateCreation)
            i.save()
...
class UpdateInfo(TemplateView):
    ...
    def post(self, request, **kwargs):
        ...
        try:
            Info.objects.filter(id=formid).update(recurrent=recurrent, facture=facture, marge=marge,
                                                  dateCloture=dateCloture, dateCreation=dateCreation, cli_id=clientid,
                                                  typ_id=typeid, partenaire_id=partenaireid, etat_id=etatid)
...
class FilterRecurrentView(TemplateView):
    ...
    def post(self, request, **kwargs):
        ...
        if finstall != '':
            for e in to_send:
                if e.dateInstallation == None or e.dateInstallation > datetime.datetime.strptime(finstall,
                                                                                                 '%Y-%m-%d').date():
                    exclude.append(e.id)
        if dinstall != '':
            for e in to_send:
                if e.dateInstallation == None or e.dateInstallation < datetime.datetime.strptime(dinstall,
                                                                                                 '%Y-%m-%d').date():
                    exclude.append(e.id)
        if fcloture != '':
            for e in to_send:
...
class ExportView(TemplateView):
    ...
    def get(self, request, **kwargs):
        ...
        if urlparse(request.META.get('HTTP_REFERER')).path != '/recurrent/':
            writer.writerow(
                ['Client', 'Partenaire', 'Type', 'Marge', 'Recurrent', 'Etat', 'Facturé', 'Date de création',
                 'Date de cloture'])
            infos = Info.objects.all().values_list('cli__nom', 'partenaire__nom', 'typ__nom', 'marge', 'recurrent',
                                                   'etat', 'facture', 'dateCreation', 'dateCloture')
            for info in infos:
                writer.writerow(info)
            return response
        else:
            writer.writerow(['Client', 'Partenaire', 'Type', 'Recurrent', 'Date de cloture', 'Date d\'installation'])
            infos = Info.objects.all().values_list('cli__nom', 'partenaire__nom', 'typ__nom', 'recurrent',
                                                   'dateCloture', 'dateInstallation')
            for info in infos:
                writer.writerow(info)
            return response
